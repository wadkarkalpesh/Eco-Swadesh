...
export default function BuyerLoginScreen() {
  ...
  const handleSendOTP = async () => {
    ...
    try {
      ...
      if (res && res.success) {
        ...
        setStep(2);
        Alert.alert('OTP Sent', `Verification code sent to ${phoneOrEmail}. (Simulated OTP: 123456)`);
      }
    } catch (e) {
    ...
    try {
      ...
      if (res && res.success) {
        ...
        if (res.isExistingUser && res.onboardingCompleted) {
          ...
          Alert.alert('OTP Verified!', 'Please complete your personal information onboarding.');
          router.push('/auth/onboarding?persona=consumer');
        }
      }
    } catch (e) {
  ...
  return (
    <ScreenContainer maxWidth="auth" withSafeArea={true}>
      {/* Banner Header */}
      <Card bg="#1976D2" style={styles.headerCard}>
        <View style={styles.bannerRow}>
          <Ionicons name="cart" size={36} color={safeTextLight} />
          <View style={{ marginLeft: 12, flex: 1 }}>
            <Text style={styles.headerTitle}>Customer & Buyer Authentication</Text>
            <Text style={styles.headerSub}>Buy Certified Organic Inputs & Direct Farm Produce</Text>
          </View>
        </View>
      </Card>
      ...
      <Card style={styles.authCard}>
        <View style={styles.stepRow}>
          <Badge label={`STEP ${step} OF 3`} variant="trust" size="sm" />
          <Badge label={step === 1 ? 'LOGIN / SIGNUP' : 'VERIFY OTP'} variant="gold" size="sm" />
        </View>
        ...
        >
          <Text style={{ fontSize: 12, color: '#1976D2', fontWeight: '700' }}>
...
const styles = StyleSheet.create({
  ...
  headerSub: { fontSize: 11, color: '#E3F2FD', marginTop: 2 },
  ...
  activeToggleBtn: { backgroundColor: '#1976D2' },
